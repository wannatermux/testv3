...
// JA3 fingerprints для различных браузеров
...
// TLS Ciphers для имитации браузера
const tlsCiphers = [
...
class Messages {
    Alert() {
        log('Hybrid [ v1.0.3 ] with TLS Fingerprinting')
...
if (process.argv.length < 7) {
    ...
    log(' --fingerprint [ use random TLS fingerprint ]')
    // --------------------------
    log('Settings -')
    ...
    log(' ./hybrid https://localhost.com 120 20 64 proxy.txt --fingerprint')
    process.exit();
}
...
const spoof = argz["s"];
const extra = argz["e"];
const useFingerprint = argv["fingerprint"];
...
const headerBuilder = {
    ...
    Sec: {
        dest: ["document", "sharedworker", "worker"],
...
function http2run() {
    ...
    Socker.HTTP(proxyOptions, (connection, error) => {
        ...
        // Настройка TLS с улучшенным fingerprinting
        const tlsOptions = {
            secure: true,
        ...
        // Добавляем случайный cipher suite если включен fingerprinting
        if (useFingerprint) {
            tlsOptions.ciphers = randomElement(tlsCiphers);
        }
...
if (cluster.isPrimary) {
    ...
    if (useFingerprint) {
        log("INFO".cyan + "  " + `TLS fingerprinting enabled`.green);
    }
